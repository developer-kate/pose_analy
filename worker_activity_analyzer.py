...
class WorkerActivityAnalyzer:
    ...
    def analyze_frame_behavior(self, landmarks, behavior):
        ...
        # 몸통 위치 분석
        torso_pos = self.analyze_torso_position(landmarks)
        if torso_pos:
            behavior['torso_position'].append(torso_pos)
        
        # 머리 방향 분석
        head_dir = self.analyze_head_direction(landmarks)
        if head_dir:
            behavior['head_direction'].append(head_dir)
    ...
    def analyze_hand_activity(self, landmarks):
        ...
        work_area_min = shoulder_y - 0.3
        work_area_max = shoulder_y + 0.3
    ...
    def analyze_torso_position(self, landmarks):
        """몸통 위치를 분석합니다."""
    ...
    def analyze_head_direction(self, landmarks):
        """머리 방향을 분석합니다."""
    ...
    def detect_precision_work(self, landmarks):
        ...
        visible_fingers = [f for f in finger_landmarks if f and f['visibility'] > 0.7]
        ...
        if len(visible_fingers) >= 4:
            ...
            if distances and np.mean(distances) < 0.06:
                return True
    ...
    def create_natural_description(self, behavior, start_str, end_str):
        ...
        torso_position = Counter(behavior['torso_position'])
        head_direction = Counter(behavior['head_direction'])
        ...
        main_torso_position = torso_position.most_common(1)[0][0] if torso_position else None
        main_head_direction = head_direction.most_common(1)[0][0] if head_direction else None
        ...
        # 작업 영역 활동
        if work_area_ratio > 0.3:
            description_parts.append("작업대에서 체계적으로 작업")
        
        # 몸 자세
        if main_torso_position:
            description_parts.append(main_torso_position)
        
        if main_head_direction:
            description_parts.append(main_head_direction)
...
def main():
    ...
    # 분석기 생성
    analyzer = WorkerActivityAnalyzer(json_file_path, fps=30)
    
    # 결과 파일 경로
    output_file_path = '../analysis/worker_activity_description.txt'
    
    # 자연스러운 설명 생성 및 저장
    description = analyzer.save_natural_description(output_file_path, segment_duration=7)
