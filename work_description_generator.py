...
class WorkDescriptionGenerator:
    ...
    def analyze_work_activity(self, start_frame, end_frame):
        ...
        for frame_idx in range(start_frame, min(end_frame, len(self.frame_data))):
            ...
            # 각 활동 분석
            self.analyze_frame_activity(landmarks, activity)
        
        return activity
    
    def analyze_frame_activity(self, landmarks, activity):
        """단일 프레임의 활동을 분석합니다."""
        # 팔 움직임 분석
        ...
        if left_arm:
            activity['arm_movements'].append(left_arm)
        if right_arm:
            activity['arm_movements'].append(right_arm)
        
        # 손 활동 분석
        hand_activity = self.analyze_hand_activity(landmarks)
        if hand_activity:
            activity['hand_activities'].append(hand_activity)
        
        # 몸통 움직임 분석
        torso_movement = self.analyze_torso_movement(landmarks)
        if torso_movement:
            activity['torso_movements'].append(torso_movement)
        
        # 머리 움직임 분석
        head_movement = self.analyze_head_movement(landmarks)
        if head_movement:
            activity['head_movements'].append(head_movement)
        
        # 정밀 작업 지표
        if self.detect_precision_work(landmarks):
            activity['precision_indicators'] += 1
        
        # 작업 영역 사용
        if self.detect_work_area_usage(landmarks):
            activity['work_area_usage'] += 1
        
        # 움직임 패턴
        movement_pattern = self.detect_movement_pattern(landmarks)
        if movement_pattern:
            activity['movement_patterns'].append(movement_pattern)
    ...
    def analyze_hand_activity(self, landmarks):
        ...
        work_area_min = shoulder_y - 0.35
        work_area_max = shoulder_y + 0.35
    ...
    def detect_precision_work(self, landmarks):
        ...
        visible_fingers = [f for f in finger_landmarks if f and f['visibility'] > 0.8]
        ...
        if len(visible_fingers) >= 4:
            ...
            if distances and np.mean(distances) < 0.05:
                return True
    ...
    def detect_movement_pattern(self, landmarks):
        ...
        if landmarks['left_wrist'] and landmarks['right_wrist']:
            ...
            # 양손이 비슷한 높이에 있는지 확인
            if abs(left_wrist_y - right_wrist_y) < 0.15:
                return "양손을 조화롭게 사용"
    ...
    def generate_work_description(self, segment_duration=7):
        ...
        for segment_start in range(0, total_frames, frames_per_segment):
            ...
            # 해당 구간의 활동 분석
            activity = self.analyze_work_activity(segment_start, segment_end)
            
            # 자연스러운 설명 생성
            description = self.create_work_description(activity, start_str, end_str)
            descriptions.append(description)
        
        return "\n".join(descriptions)
    
    def create_work_description(self, activity, start_str, end_str):
        """활동 분석을 기반으로 작업 설명을 생성합니다."""
        # 주요 활동 파악
        arm_movements = Counter(activity['arm_movements'])
        hand_activities = Counter(activity['hand_activities'])
        torso_movements = Counter(activity['torso_movements'])
        head_movements = Counter(activity['head_movements'])
        ...
        # 비율 계산
        total_frames = len(activity['arm_movements']) + len(activity['hand_activities'])
        precision_ratio = activity['precision_indicators'] / total_frames if total_frames > 0 else 0
        work_area_ratio = activity['work_area_usage'] / total_frames if total_frames > 0 else 0
        
        # 작업 유형 결정
        work_type = self.determine_work_type(activity)
    ...
    def determine_work_type(self, activity):
        """활동 분석을 기반으로 작업 유형을 결정합니다."""
        # 정밀 작업 비율
        total_frames = len(activity['arm_movements']) + len(activity['hand_activities'])
        precision_ratio = activity['precision_indicators'] / total_frames if total_frames > 0 else 0
        ...
        # 팔 활동 분석
        arm_text = " ".join(activity['arm_movements']).lower()
        
        if precision_ratio > 0.2:
            return "정밀 조립 작업"
        elif "들어올림" in arm_text or "내림" in arm_text:
            return "부품 조작 작업"
        elif work_area_ratio > 0.35:
            return "조립 작업"
        else:
            return "일반 작업"
...
def main():
    ...
    # 생성기 생성
    generator = WorkDescriptionGenerator(json_file_path, fps=30)
    
    # 결과 파일 경로
    output_file_path = '../analysis/work_description.txt'
    
    # 작업 설명 생성 및 저장
    description = generator.save_work_description(output_file_path, segment_duration=7)
